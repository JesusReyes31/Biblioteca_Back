...
import { getBook, getBooks, postBook, putBook, deleteBook, getBooksDisponibles, getAllBooks} from "../controllers/books.controller";
import {checkAuth, checkRole} from "../middleware/session";
const router = Router();

router.get('/:idSuc',getBooks)
router.get('/:id/:idSuc',getBook)
router.get('/get/all/books',checkAuth,checkRole(['Inventario']),getAllBooks)
